...
def check_json(response: str) -> Tuple[bool, dict]:
    try:
        ...
        if (response['to'] == 'user'):
            # FormatAgent only returns to user, no function calls
            assert ('text' in response)
            # FormatAgent's text should be a JSON object (the final response)
            assert (type(response['text']) == dict)
        else:
            # FormatAgent should not call functions
            assert (False), "FormatAgent should not call functions"
